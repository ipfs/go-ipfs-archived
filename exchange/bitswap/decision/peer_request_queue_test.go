...
func TestPushPop(t *testing.T) {
	...
	for _, index := range rand.Perm(len(alphabet)) { // add blocks for all letters
		...
		ledger := newLedger(partner)
		prq.Push(&wantlist.Entry{Cid: c, Priority: math.MaxInt32 - index}, ledger)
	}
...
func TestPeerRepeats(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		prq.Push(&wantlist.Entry{Cid: elcid}, newLedger(a))
		prq.Push(&wantlist.Entry{Cid: elcid}, newLedger(b))
		prq.Push(&wantlist.Entry{Cid: elcid}, newLedger(c))
		prq.Push(&wantlist.Entry{Cid: elcid}, newLedger(d))
	}
